...
export type Path<Pattern extends string> = {
  ...
  path: <Subpattern extends string>(
...
export function path<Pattern extends string>(pattern: Pattern): Path<Pattern> {
  ...
  const paramsToPath: Path<Pattern> = (params) => {
    return (
      "/" +
      parts
        .map((part) => {
  ...
  if (!paramsToPath.pattern.startsWith("/")) {
    throw new Error(
      `Paths must begin with slashes, but ${JSON.stringify(
        paramsToPath.pattern
      )} doesn't.`
    );
  }
...
function patternParts<Pattern extends string>(
  pattern: Pattern
): Part<Pattern>[] {
  return pattern.split("/").map((part) => {
    if (part.startsWith(":")) {
      return {
        kind: "param",
        paramName: part.replace(/^:/, "") as PathParamNames<Pattern>,
      };
    } else {
 ...
 * match manually. */
function normalizePattern<Pattern extends string>(
  pattern: Pattern
  ...
  return (
    pattern
      ...
      .replace(
        /^(.+)\/$/,
        (match, patternWithoutTrailingSlash) =>
          patternWithoutTrailingSlash as NormalizePattern<Pattern>
      ) as NormalizePattern<Pattern>
  );
}

function paramValue<Pattern extends string>(
